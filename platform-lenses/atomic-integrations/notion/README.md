...
```sh
esbuild platform-lenses/atomic-integrations/notion/plugin.ts --bundle --format=esm --platform=neutral --target=es2022 --outfile=platform-lenses/atomic-integrations/notion/plugin.js
vitest run --config platform-lenses/atomic-integrations/notion/vitest.config.ts
```
...
```sh
ATOMIC_NOTION_TEST_SERVER=http://localhost:9898 vitest run --config platform-lenses/atomic-integrations/notion/vitest.config.ts
```
